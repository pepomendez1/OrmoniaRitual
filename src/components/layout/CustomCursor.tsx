...
export function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const onOver = (e: PointerEvent) => {
      ...
      gsap.to(dot, { opacity: expand ? 0 : 1, duration: 0.3 });
    };
  ...
  return (
    <>
      <div
        ref={ringRef}
        aria-hidden="true"
        className="pointer-events-none fixed left-0 top-0 z-[9999] h-7 w-7 rounded-full border border-foreground/40 opacity-0"
      />
      <div
        ref={dotRef}
        aria-hidden="true"
        className="pointer-events-none fixed left-0 top-0 z-[9999] h-1.5 w-1.5 rounded-full bg-foreground opacity-0"
      />
    </>
  );
}
